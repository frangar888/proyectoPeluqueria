 ...
 * @return {JSDataSet} A dataset with 1 or 2 columns display[,real]
 *
 * @properties={typeid:24,uuid:"22F39206-DDFC-485D-9B26-F021FB28A8AA"}
 */
function getDataSetForValueList_adn_vendedores_activos(displayValue, realValue, record, valueListName, findMode, rawDisplayValue) {
	var args = new Array()
	/** @type  {JSDataSet} */
	var result = null;
	if (displayValue == null && realValue == null) 
	{
		result =  databaseManager.getDataSetByQuery("peluqueria", "select concat(cast(a.ven_codigo as char),'-',b.adn_nombre), a.adn_id from adn_vendedores as a inner join adn as b on a.adn_id = b.adn_id where a.ven_estado = 0 order by ven_codigo asc ", args, -1);
	} 
	else if (displayValue != null) 
	{
		// TYPE_AHEAD filter call, return a filtered list
		args = [, "%" + displayValue + "%",displayValue + "%"]
		result =  databaseManager.getDataSetByQuery("peluqueria", "select concat(cast(a.ven_codigo as char),'-',b.adn_nombre), a.adn_id from adn_vendedores as a inner join adn as b on a.adn_id = b.adn_id where a.ven_estado = 0 and (a.ven_codigo like ? OR b.adn_nombre like ?) order by ven_codigo asc", args, -1);
	...
	else if (realValue != null) 
	{
		...
		// dont return a complete list in this mode because that will be added to the list that is already there
		args = [realValue];
		result =  databaseManager.getDataSetByQuery("peluqueria", "select concat(cast(a.ven_codigo as char),'-',b.adn_nombre), a.adn_id from adn_vendedores as a inner join adn as b on a.adn_id = b.adn_id where a.ven_estado = 0 and a.adn_id = ?", args, -1);
